...
$listproj = "SELECT * FROM `project` WHERE `projectId`='$xprojId'";
$result_proj = mysqli_query($dbc, $listproj);
$row = mysqli_fetch_assoc($result_proj);

$listmem = "SELECT * FROM `member` WHERE `projectId`='$xprojId'";
$result_mem = mysqli_query($dbc, $listmem);
$row3 = mysqli_fetch_assoc($result_mem);

if ($row) {
    // Query to retrieve members associated with the project and their related personnel information

    $list_EC = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `editor_coordinator` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members2 = mysqli_query($dbc, $list_EC);
    $row2 = mysqli_fetch_assoc($result_members2);

    $list_EP = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `editor_proofing` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members4 = mysqli_query($dbc, $list_EP);
    $row4 = mysqli_fetch_assoc($result_members4);

    $list_DT = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `designer_text` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members5 = mysqli_query($dbc, $list_DT);
    $row5 = mysqli_fetch_assoc($result_members5);

    $list_DC = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `designer_cover` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members6 = mysqli_query($dbc, $list_DC);
    $row6 = mysqli_fetch_assoc($result_members6);

    $list_IT = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `illustrator_text` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members7 = mysqli_query($dbc, $list_IT);
    $row7 = mysqli_fetch_assoc($result_members7);

    $list_IC = "SELECT m.*, p.* FROM `member` m
                 INNER JOIN `illustrator_cover` EC ON m.`memberId` = EC.`memberId`
                 INNER JOIN `personnel` p ON EC.`personnelId` = p.`personnelId`
                 WHERE m.`projectId`='$xprojId'";
    $result_members8 = mysqli_query($dbc, $list_IC);
    $row8 = mysqli_fetch_assoc($result_members8);
} else {
...
<body style="background: #ffe5ec;">
    ...
    <div class="container">
        <div id="main" class="card">
            ...
            <form action="updateproject.php?id=<?php echo $xprojId; ?>" method="post" enctype="multipart/form-data" class="css-dropdown">
                <div class="form-group col-md-13">
                    <label for="projectTitle">Project Title</label>
                    <input type="text" class="form-control" id="projectTitle" name="projectTitle" value="<?= $row['title'] ?>" required />
                </div>
                ...
                <table width="100%">
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="text" class="form-control" id="projectSiri" name="projectSiri" value="<?= $row['siri'] ?>" />
                        </td>
                    </tr>
                    <tr>
                        <td><label for="projectSize">Project Size</label>
                            <select class="form-control" id="projectSize" name="projectSize">
                                <option value="<?= $row['projectSize'] ?>" selected><?= $row['projectSize'] ?>
                                </option>
                        ...
                        <td><label for="totalPages">Total Pages</label>
                            <input type="number" class="form-control" id="totalPages" name="totalPages" min="1" value="<?= $row['totalPages'] ?>" />
                        </td>
                    </tr>
                    <tr>
                        <td><label for="typeDesign">Type of Design</label>
                            <select class="form-control" id="typeDesign" name="typeDesign" required>
                                <option value="<?= $row['typeOfDesign'] ?>" selected><?= $row['typeOfDesign'] ?>
                                </option>
                        ...
                        <td><label for="Finishing">Finishing</label>
                            <select class="form-control" id="Finishing" name="Finishing">
                                <option value="<?= $row['typeOfFinishing'] ?>" selected><?= $row['typeOfFinishing'] ?>
                                </option>
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="text" class="form-control" id="Writer" name="Writer" value="<?= $row3['writerName'] ?>" />
                </div>
                <table width="100%">
                    <tr>
                        <td><label for="coordinator">Editor (Coordinator)</label>
                            <select class="form-control" id="coordinator" name="coordinator" required>
                                <option value="<?= $row2['personnelId'] ?>" selected><?= $row2['name'] ?></option>
                                <?php
                        ...
                        <td><label for="proofing">Editor (Proofing)</label>
                            <select class="form-control" id="proofing" name="proofing" required>
                                <option value="<?= $row4['personnelId'] ?>" selected><?= $row4['name'] ?></option>
                                <?php
                                foreach ($qpersonnels as $personnel) :
                                    if ($personnel["position"] == "Editor") {
                                        echo "<option value='" . $personnel["personnelId"] . "'>" . $personnel["name"] . "</option>";
                    ...
                    <tr>
                        <td><label for="designerText">Graphic Designer (Text)</label>
                            <select class="form-control" id="designerText" name="designerText" required>
                                <option value="<?= $row5['personnelId'] ?>" selected><?= $row5['name'] ?></option>
                                <?php
                                foreach ($qpersonnels as $personnel) :
                        ...
                        <td><label for="designerCover">Graphic Designer (Cover)</label>
                            <select class="form-control" id="designerCover" name="designerCover">
                                <option value="<?= $row6['personnelId'] ?>" selected><?= $row6['name'] ?></option>
                                <?php
                                foreach ($qpersonnels as $personnel) :
                                    if ($personnel["position"] == "Graphic Designer") {
                                        echo "<option value='" . $personnel["personnelId"] . "'>" . $personnel["name"] . "</option>";
                    ...
                    <tr>
                        <td><label for="IlusText">Illustrator (Text)</label>
                            <select class="form-control" id="IlusText" name="IlusText" required>
                                <option value="<?= $row7['personnelId'] ?>" selected><?= $row7['name'] ?></option>
                                <?php
                                foreach ($qpersonnels as $personnel) :
                        ...
                        <td><label for="IlusCover">Illustrator (Cover)</label>
                            <select class="form-control" id="IlusCover" name="IlusCover">
                                <option value="<?= $row8['personnelId'] ?>" selected><?= $row8['name'] ?></option>
                                <?php
                                foreach ($qpersonnels as $personnel) :
                                    if ($personnel["position"] == "Illustrator") {
                                        echo "<option value='" . $personnel["personnelId"] . "'>" . $personnel["name"] . "</option>";
                ...
                <div class="form-group col-md-13">
                    ...
                    <input type="number" class="form-control" id="IlusPages" name="IlusPages" min="1" value="<?= $row['IlusPages'] ?>" />
                </div>
                <br>

                <button type="submit" name="submit" class="btn btn-primary">Edit Details</button>
